...
Problem rand_problem (size_t _nIndices, size_t _nQueries, Real _sizeRatio = 4, Real _fixedCostRatio = 0.1, Real _maxMinRatio = 5, Real _unindexedIndexedRatio = 1.5, unsigned int _seed = 0)
{
	...
	Problem problem{};
	{
		...
		problem.unindexedQueryCosts = ImmutableArray<Real>::takeOwnership (ucs, _nQueries);
	}
	{
		...
		for (int i{ 0 }; i < _nIndices; i++)
		{
			...
			index.queryCosts = ImmutableArray<Real>::takeOwnership (cs, _nQueries);
			index.fixedCost = r (gen) * _fixedCostRatio;
			index.size = r (gen);
		}
		problem.indices = ImmutableArray<Index>::takeOwnership (idxs, _nIndices);
